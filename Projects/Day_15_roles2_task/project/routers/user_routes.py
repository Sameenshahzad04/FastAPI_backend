...
from handlers.user_handler import get_user_by_email,create_user,authenticate_user,show_all_users,delete_user
...
def userReg(u:UserRegister,db:Session=Depends(get_db)):
    ...
    return create_user(db,u.username,u.email,hashed_password,u.role)
...
def show_users(db: Session = Depends(get_db), current_user: User = Depends(role_required("admin"))):
    ...
    return show_all_users(db) 
# @user_routes.post("/Create_user",response_model=Userout)
# def to_create_user(u:UserRegister,db: Session = Depends(get_db), current_user: User = Depends(role_required("admin")),):
#     d_user=get_user_by_email(db,u.email)
#     if d_user:
#         raise HTTPException(status_code=status.HTTP_208_ALREADY_REPORTED,detail="user already register")
#     hashed_password = get_pwd_hash(u.password)
